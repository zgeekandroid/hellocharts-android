...
public class LineChartActivity extends ActionBarActivity {
    ...
    public static class PlaceholderFragment extends Fragment {
        ...
        public View onCreateView(LayoutInflater inflater, ViewGroup container, Bundle savedInstanceState) {
            ...
            generateChartsBottom(rootView);
            return rootView;
        }


        public void generateChartsBottom(View rootView){
            LinearLayout  view = (LinearLayout) rootView.findViewById(R.id.chart_bottom);
            for (int i = 0; i < data.getLines().size(); i++) {
                final ChartTagView tagView = new ChartTagView(getActivity());

                if (i != 0){
                    tagView.setChartLine(data.getLines().get(i));
                }

                tagView.setLabelText("消费总金额(单位)" + i);
                tagView.setValueText(String.valueOf((i + 1) * 123.0F));
                tagView.setLabelSize(40);
                LinearLayout.LayoutParams params = new LinearLayout.LayoutParams(ViewGroup.LayoutParams.MATCH_PARENT, ViewGroup.LayoutParams.WRAP_CONTENT);
                params.weight = 1;
                params.gravity = Gravity.CENTER;
                tagView.setLayoutParams(params);
                tagView.setOnToggleListener(new ChartTagView.onToggleListener() {
                    @Override
                    public void onToggle(boolean toggle) {
                        if (toggle) {
                            numberOfLines = numberOfLines + 1;
                            data.getLines().add(tagView.getLine());
                        } else {
                            data.getLines().remove(tagView.getLine());
                            numberOfLines = numberOfLines - 1;
                        }
        ...
        private void generateData() {
            ...
            if (hasAxes) {
//                Axis axisX = new Axis();
                Axis axisX = Axis.generateAxisFromRange(0,20,5);
